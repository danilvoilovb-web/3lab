...
const renderGallery = () => {
  ...
  for (const box of boxes) {
    const regionKey = box.dataset.region;
    const itemIndex = parseInt(box.dataset.index);
...
const initMobileMenu = () => {
  ...
  if (burgerBtn && navList) {
    burgerBtn.addEventListener('click', () => {
      ...
      navList.classList.toggle('menu__list--active');
    });
  }
};
...
const initFooterLinks = () => {
  ...
  const tabMap = { 'france': '#tab-1', 'germany': '#tab-2', 'england': '#tab-3' };
  ...
  footerLinks.forEach(link => {
    link.addEventListener('click', (e) => {
      ...
      if (tabToClick) {
        tabToClick.click();
        document.querySelector('#reproduction').scrollIntoView({ behavior: 'smooth' });
      }
    });
  });
};
...
const initPopup = () => {
  ...
  const closeBtnBottom = document.querySelector('.popup__btn-close');
  ...
  // Закрытие по крестику, кнопке или фону
